...
import { useCreateJobPosition } from "../../../hooks/api/jobPosition/useCreateJobPosition";
...
const schema = object({
    ...
    jobTitle: string().required(requiredErrorText).max(64).min(3),
    companyName: string().required(requiredErrorText).max(64).min(3),
    ...
    contactEmail: string().required(requiredErrorText).max(300).min(3),
...
const getHourSalary = (
    ...
    if (
        hourSalary === 0 ||
        (typeof hourSalary === "number" && isNaN(hourSalary))
    ) {
        return "Po domluvě";
    }
...
export default function Form() {
    ...
    const { mutate: createJobPosition, isPending } = useCreateJobPosition({
        onSuccess: () => {
            reset();
            toast({
                title: `Pracovní pozice byla odeslána na Váš email: ${watchEmail} ke schválení. Prosím zkontrolujte i složku spam.`,
                status: "success",
                duration: 9000,
                isClosable: true,
            });
